...
export const encodeInstruction = (instruction: Instr): bigint => {
  ...
  const paddedCharCodes = charCodes.map((charCode) =>
    charCode.toString().padStart(ENCODED_CHAR_SIZE, '0')
  );
...
export const decodeInstruction = (encodedInstruction: bigint): Instr => {
  ...
  const encodedInstructionSubstring = encodedInstruction
    .toString()
    .substring(1);
  const paddedCharCodes: string[] = [];
  for (
    let i = 0;
    i < encodedInstructionSubstring.length;
    i += ENCODED_CHAR_SIZE
  ) {
    paddedCharCodes.push(
      encodedInstructionSubstring.substring(i, i + ENCODED_CHAR_SIZE)
    );
  }
  const charCodes = paddedCharCodes.map((paddedCharCode) =>
    parseInt(paddedCharCode)
  );
